...
std::string FrameProcessor::getModeName(int mode) {
    switch (mode) {
        ...
        case 6: return "Face Detection";
        default: return "Custom Mode";
    }
}

void FrameProcessor::process(cv::Mat& frame, int mode, float zoom, double angle, const cv::Rect& mouseRect, const std::vector<cv::Rect>& faces) {
